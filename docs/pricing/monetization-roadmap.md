...
The cleanest next OpenSpec proposal is:

1. `define-monetization-contract` if policy questions still need to be frozen
2. otherwise `persist-usage-telemetry`

If the goal is to get the foundation right before customer-facing billing, the first implementation change should not be Stripe checkout. It should be the internal telemetry and billing domain groundwork that makes Stripe integration safe and auditable.
