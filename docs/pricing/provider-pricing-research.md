...
| Automatic retry behavior                  | One automatic retry on max-token failure when `maxTokens` was not user-specified | Retry cost belongs to the same successful request boundary and now rolls into `generation_runs.attempt_count` plus request token totals when every attempt exposes observed usage. | `backend/src/experience/services/experience-orchestrator.service.ts`                                                                   |
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `assistant_runs`      | Conversation lifecycle, selected routing tier/provider/model, raw provider request/response, conversation provider/model, aggregated `conversation_tokens_in` / `conversation_tokens_out` | Conversation overhead, assistant-run status, queryable run-level conversation cost   | Not a billable unit by itself; raw provider envelopes still overwrite by round                                   |
| `tool_invocations`    | Per-tool execution status, tool args/result, router provider/model, router request/response traces, router tokens, selected provider/model, generation/version correlation | The correct place to count successful `generate_experience` invocations and inspect per-execution routing cost | No explicit retry-attempt log; multiple successes can happen inside one assistant run                            |
| `generation_runs`     | Generation request lifecycle, operation, provider/model, final status, output shape summary, `attempt_count`, request token totals | Request-level success/failure counts, retry-aware cost, and operation segmentation    | No per-attempt event table; request token totals stay null when any attempt lacks observed usage                 |
| `experience_versions` | Persisted artifact, provider/model, quality mode, latency, `tokens_in`, and `tokens_out`                                | Successful artifact evidence with artifact-producing token counts when observed       | Historical rows before the telemetry rollout can still have null token columns                                   |
...
- Current cost work can proceed with official pricing and one real observed run, but any launch recommendation must still be labeled preview-sensitive and assumption-heavy until post-rollout telemetry volume replaces the current one-run planning sample.
