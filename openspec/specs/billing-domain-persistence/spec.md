# billing-domain-persistence Specification

## Purpose

Durable storage and integrity constraints for Monti billing entities (customer mapping, subscription mirror, checkout audit trail, webhook idempotency, credit grants, reservations, append-only ledger, pricing rule snapshots) plus backend configuration for feature flags, so payment integration and runtime settlement can attach to a single auditable model.
