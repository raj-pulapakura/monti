# conversation-credit-gate Specification

## Purpose
Define API-layer credit checks on chat message submission: reject when the user cannot afford the selected mode, optional silent auto→fast downgrade, and alignment with credit enforcement flags.
...
- **THEN** the run is queued with effective mode `fast`, no error is returned, and the downgrade is recorded in structured application logs
...
- **THEN** the run is queued with effective mode resolved per existing routing (e.g. quality) without error
