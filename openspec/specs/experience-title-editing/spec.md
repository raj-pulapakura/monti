# experience-title-editing Specification

## Purpose

The experience title lives on the `experiences` record (not on versions). Educators can rename it from the thread sandbox without changing slug or version history.
...
- **THEN** a pencil/edit icon is rendered next to the experience title
...
- **THEN** the title display is replaced by a text input pre-filled with the current title, and save (✓) and cancel (✕) controls appear
