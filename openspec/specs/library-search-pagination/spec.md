...
The library section SHALL provide a text input that filters the visible creation cards client-side. Filtering SHALL match against each card's display title (preferring `experienceTitle`, falling back to `thread.title`) using a case-insensitive substring match. The full thread list (up to the fetched limit) is always available for filtering regardless of the current page.
...
- **WHEN** the search query does not match any card's display title
- **THEN** the grid renders an explicit empty state: "No creations match \"{query}\"." and no pagination controls are shown
...
- **THEN** the full unfiltered thread list is shown again and pagination resets to page 1
